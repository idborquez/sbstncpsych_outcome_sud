...
treat_1 <- tableby(treat_time_t1_v1 ~ factor(poly_oh_coc) +
...
# Pr actual treatment
density_plot_t1_1 <- density(df_outcome$pr_tt_t1[df_outcome$treat_time_t1_v1 == "3 or less months" & !is.na(df_outcome$w_t_595)], na.rm = TRUE, bw = 0.01)
density_plot_t1_2 <- density(df_outcome$pr_tt_t1[df_outcome$treat_time_t1_v1 == "4 to 7 months" & !is.na(df_outcome$w_t_595)], na.rm = TRUE, bw = 0.01)
density_plot_t1_3 <- density(df_outcome$pr_tt_t1[df_outcome$treat_time_t1_v1 == "8 or more months" & !is.na(df_outcome$w_t_595)], na.rm = TRUE, bw = 0.01)
density_plot_t1_4 <- density(df_outcome$pr_tt_t1[df_outcome$treat_time_t1_v1 == "Currently in treatment" & !is.na(df_outcome$w_t_595)], na.rm = TRUE, bw = 0.01)
...
##### Table weights
```{r eval=FALSE}
weights <- tableby(treat_time_t1_v1 ~ pr_notcen_t2 + ipcw_t2 + pr_tt1_t1 + pr_tt2_t1 + pr_tt3_t1 + pr_tt4_t1 + iptw_tt_t1 + w_t + w_t_595, 
                  data = df_outcome, numeric.stats=c("mean", "sd", 
                                                    "medianq1q3", 
                                                    "min", "max", "Nmiss", "N"),
                  digits=3, digits.p=3, digits.pct=1, na.action(TRUE))
...
df_outcome <- df_outcome[!is.na(df_outcome$w_t_595), ]

# Create weighted data
df_outcome_Svy <- svydesign(ids = ~ 1, data = df_outcome, weights = ~ w_t_595)
...
# Pr actual treatment
density_plot_t1_1 <- density(df_outcome$pr_trc_t1[df_outcome$treat_comp_t1_v1 == "Not Completed" & !is.na(df_outcome$w_t_595)], na.rm = TRUE, bw = 0.01)
density_plot_t1_2 <- density(df_outcome$pr_trc_t1[df_outcome$treat_comp_t1_v1 == "Completed" & !is.na(df_outcome$w_t_595)], na.rm = TRUE, bw = 0.01)
density_plot_t1_3 <- density(df_outcome$pr_trc_t1[df_outcome$treat_comp_t1_v1 == "Currently in treatment" & !is.na(df_outcome$w_t_595)], na.rm = TRUE, bw = 0.01)
...
##### Table weights
```{r eval=FALSE}
weights <- tableby(treat_comp_t1_v1 ~ pr_notcen_t2 + ipcw_t2 + pr_tc1_t1 + pr_tc2_t1 + pr_tc3_t1  + pr_trc_t1+ iptw_tc_t1 + w_t + w_t_595, 
                  data = df_outcome, numeric.stats=c("mean", "sd", 
                                                    "medianq1q3", 
                                                    "min", "max", "Nmiss", "N"),
                  digits=3, digits.p=3, digits.pct=1, na.action(TRUE))
...
## Outcome model: Treatment complaince
