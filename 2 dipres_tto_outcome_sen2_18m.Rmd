---
title: "DIPRES: outcomes substance use and mental health: sensitivity updates outcomes"
...
a <- table(df$treat_time_t2_v1, useNA = "ifany")
round(prop.table(a)*100,2) 
...
# NA at time 2
subset_data <- df$treat_time_t2_v1[df$t2 == 1]
...
a <- table(df$treat_comp_t2_v1, useNA = "ifany")
round(prop.table(a)*100,2)

a <- table(df$treat_comp_t2_v1)
round(prop.table(a)*100,2)
```
# Outcomes
...
table(df_cal_2$diff_1)

merged_df <- merge(df, df_cal_2, by = "folio", all.x = TRUE)
table(merged_df$tto_30dias_t0, merged_df$diff_1, useNA = "ifany")
```
...
# 3 More than 3 months exposed to treatment

# Lifetime SUD treatment
table(df$tto_primertrata_t0)

# Motivation to change
data_cronbach <- df[,c(344:375)]
...
# Total number of physicians, psychiatrist, psychologist, social workers, nurses, occupational therapist
...
# Take only the two variables
df_c1 <- df_c[,c(2,82,270)]
```

# Merge
```{r}
# See how terrible the non-overlap is
df_subset <- df[c(19,23,24)]

df_subset <- df_subset %>%
  distinct(folio_centro, .keep_all = TRUE)

df_subset <- merge(df_subset,df_c1,by="folio_centro", all = TRUE, incomparables = T) # No problems with the folio
...
df_tables[c("treat_time_t2_v1")] <-
  lapply(df_tables[c("treat_time_t2_v1")], factor,
                                 levels=c("1", 
                                          "2",
                                          "3",
                                          "4"), 
...
df_tables[c("treat_comp_t2_v1")] <-
  lapply(df_tables[c("treat_comp_t2_v1")], factor,
                                 levels=c("1", 
                                          "2",
                                          "3"), 
...
treat_comp_t2_v1 = "Treatment compliance T2",
treat_time_t2_v1 = "Time in treatment T2",
...
# Outcome regression dataset
```{r}
df_outcome <- df_tables
```

# T-test change over time in the outcomes
```{r eval=FALSE}
...
plot(density_plot, main = "Prob Not Censored T2 Density", xlab = "Probability not censored", col = "blue", xlim = c(0, 1.1))

# Check density across censored and uncensored T2
density_plot_t2_1 <- density(df_model$pr_notcen_t2[df_model$t2 == 1], na.rm = TRUE)
density_plot_t2_0 <- density(df_model$pr_notcen_t2[df_model$t2 == 0], na.rm = TRUE)

# Plot 
plot(density_plot_t2_1, main = "Prob Not Censored T2 Density by censored", xlab = "Probability not censored", col = "blue", xlim = c(0,  1.1))
lines(density_plot_t2_0, col = "red")
...
save(df_ipcw_t2, file = "df_ipcw_t2_v2_sen2.Rdata")
```
...
treat_1 <- tableby(treat_time_t2_v1 ~ 
...
df_model <- df_model[complete.cases(df_model$treat_time_t2_v1), ]

p_missing <- unlist(lapply(df_model, function(x) sum(is.na(x))))/nrow(df_model)
round(sort(p_missing[p_missing > 0], decreasing = TRUE)*100,2)

df_model$treat <- as.numeric(df_model$treat_time_t2_v1)
...
# Perform logistic regression on the complete dataset
complete_model <- multinom(treat_time_t2_v1 ~ 
                           programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini +  drgs_sp_t0 + # Substance use
...
df_model$pr_tt_t1[df_model$treat_time_t2_v1 == "3 or less months"] <- df_model$pr_tt1_t1
df_model$pr_tt_t1[df_model$treat_time_t2_v1 == "4 to 7 months" & is.na(df_model$pr_tt_t1)] <- df_model$pr_tt2_t1
df_model$pr_tt_t1[df_model$treat_time_t2_v1 == "8 or more months" & is.na(df_model$pr_tt_t1)] <- df_model$pr_tt3_t1
df_model$pr_tt_t1[df_model$treat_time_t2_v1 == "Currently in treatment" & is.na(df_model$pr_tt_t1)] <- df_model$pr_tt4_t1
...
# Check density
density_plot_t1_1 <- density(df_model$iptw_tt_t1[df_model$treat_time_t2_v1 == "3 or less months"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$iptw_tt_t1[df_model$treat_time_t2_v1 == "4 to 7 months"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$iptw_tt_t1[df_model$treat_time_t2_v1 == "8 or more months"], na.rm = TRUE)
density_plot_t1_4 <- density(df_model$iptw_tt_t1[df_model$treat_time_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Check density
density_plot_t1_1 <- density(df_model$pr_tt1_t1[df_model$treat_time_t2_v1 == "3 or less months"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$pr_tt1_t1[df_model$treat_time_t2_v1 == "4 to 7 months"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$pr_tt1_t1[df_model$treat_time_t2_v1 == "8 or more months"], na.rm = TRUE)
density_plot_t1_4 <- density(df_model$pr_tt1_t1[df_model$treat_time_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Check density
density_plot_t1_1 <- density(df_model$pr_tt2_t1[df_model$treat_time_t2_v1 == "3 or less months"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$pr_tt2_t1[df_model$treat_time_t2_v1 == "4 to 7 months"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$pr_tt2_t1[df_model$treat_time_t2_v1 == "8 or more months"], na.rm = TRUE)
density_plot_t1_4 <- density(df_model$pr_tt2_t1[df_model$treat_time_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Check density
density_plot_t1_1 <- density(df_model$pr_tt3_t1[df_model$treat_time_t2_v1 == "3 or less months"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$pr_tt3_t1[df_model$treat_time_t2_v1 == "4 to 7 months"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$pr_tt3_t1[df_model$treat_time_t2_v1 == "8 or more months"], na.rm = TRUE)
density_plot_t1_4 <- density(df_model$pr_tt3_t1[df_model$treat_time_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Check density
density_plot_t1_1 <- density(df_model$pr_tt4_t1[df_model$treat_time_t2_v1 == "3 or less months"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$pr_tt4_t1[df_model$treat_time_t2_v1 == "4 to 7 months"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$pr_tt4_t1[df_model$treat_time_t2_v1 == "8 or more months"], na.rm = TRUE)
density_plot_t1_4 <- density(df_model$pr_tt4_t1[df_model$treat_time_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Pr actual treatment
density_plot_t1_1 <- density(df_model$pr_tt_t1[df_model$treat_time_t2_v1 == "3 or less months"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$pr_tt_t1[df_model$treat_time_t2_v1 == "4 to 7 months"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$pr_tt_t1[df_model$treat_time_t2_v1 == "8 or more months"], na.rm = TRUE)
density_plot_t1_4 <- density(df_model$pr_tt_t1[df_model$treat_time_t2_v1 == "Currently in treatment"], na.rm = TRUE)

plot(density_plot_t1_1, main = "Pr actual treatment (time in) at T1 Density", xlab = "Probability time in treatment received at T1", col = "blue", xlim = c(0,1), ylim = c(0,4))
lines(density_plot_t1_2, col = "red")
lines(density_plot_t1_3, col = "green")
lines(density_plot_t1_4, col = "gold")
legend("topright", legend = c("0 to 3", "4 to 7", "8 or more", "Currently in treatment"), col = c("blue", "red", "green", "gold"), lty = 1)
...
save(df_iptw_t1, file = "df_iptw_tt_t1_v2_sen2.Rdata")
```

## Merge df_outcome with weights
```{r}
load(file = "df_ipcw_t2_v2_sen2.Rdata")
load(file = "df_iptw_tt_t1_v2_sen2.Rdata")
...
# Check density across censored and uncensored T2
density_plot_t1_1 <- density(df_outcome$w_t[df_outcome$treat_time_t2_v1 == "3 or less months"], na.rm = TRUE)
density_plot_t1_2 <- density(df_outcome$w_t[df_outcome$treat_time_t2_v1 == "4 to 7 months"], na.rm = TRUE)
density_plot_t1_3 <- density(df_outcome$w_t[df_outcome$treat_time_t2_v1 == "8 or more months"], na.rm = TRUE)
density_plot_t1_4 <- density(df_outcome$w_t[df_outcome$treat_time_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
```{r eval=FALSE}
# Plot
density_plot <- density(df_outcome$w_t_595, na.rm = T)
plot(density_plot, main = "IPCW*IPTW time in Tx Density", xlab = "IPCW*IPTW", col = "blue")

# Check density across censored and uncensored T2
density_plot_t1_1 <- density(df_outcome$w_t_595[df_outcome$treat_time_t2_v1 == "3 or less months"], na.rm = TRUE)
density_plot_t1_2 <- density(df_outcome$w_t_595[df_outcome$treat_time_t2_v1 == "4 to 7 months"], na.rm = TRUE)
density_plot_t1_3 <- density(df_outcome$w_t_595[df_outcome$treat_time_t2_v1 == "8 or more months"], na.rm = TRUE)
density_plot_t1_4 <- density(df_outcome$w_t_595[df_outcome$treat_time_t2_v1 == "Currently in treatment"], na.rm = TRUE)

# Plot 
plot(density_plot_t1_1, main = "IPCW*IPTW time in Tx by treatment status", 
     xlab = "IPCW*IPTW", col = "blue", ylim = c(0, 1.5), xlim = c(0,max(df_outcome$w_t_595, na.rm = T)))
lines(density_plot_t1_2, col = "red")
lines(density_plot_t1_3, col = "green")
lines(density_plot_t1_4, col = "gold")
legend("topright", legend = c("0 to 3", "4 to 7", "8 or more", "Currently in treatment"), col = c("blue", "red", "green", "gold"), lty = 1)
...
# Pr actual treatment
density_plot_t1_1 <- density(df_outcome$pr_tt_t1[df_outcome$treat_time_t2_v1 == "3 or less months" & !is.na(df_outcome$w_t_595)], na.rm = TRUE)
density_plot_t1_2 <- density(df_outcome$pr_tt_t1[df_outcome$treat_time_t2_v1 == "4 to 7 months" & !is.na(df_outcome$w_t_595)], na.rm = TRUE)
density_plot_t1_3 <- density(df_outcome$pr_tt_t1[df_outcome$treat_time_t2_v1 == "8 or more months" & !is.na(df_outcome$w_t_595)], na.rm = TRUE)
density_plot_t1_4 <- density(df_outcome$pr_tt_t1[df_outcome$treat_time_t2_v1 == "Currently in treatment" & !is.na(df_outcome$w_t_595)], na.rm = TRUE)

# Plot 
plot(density_plot_t1_1, main = "Pr actual treatment (time in) at T1 Density trimmed", 
     xlab = "Pr actual treatment received", col = "blue", ylim = c(0, 4.2), xlim = c(0,1))
lines(density_plot_t1_2, col = "red")
lines(density_plot_t1_3, col = "green")
lines(density_plot_t1_4, col = "gold")
legend("topright", legend = c("0 to 3", "4 to 7", "8 or more", "Currently in treatment"), col = c("blue", "red", "green", "gold"), lty = 1)
...
# 1 Unadjusted
m_sp1 <- glm(sp_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
...
m_sp7 <- geeglm(sp_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
log <- c("tto_primertrata_t0", "tbjo_t0", "pb_mprev_t0", 
         "drgs_ini", "p42", "tranq_mprev_t0" ,"redes_vivedrogas_t0" , "mar_mprev_t0" , "sld_mini_antisoc_t0" )
...
set.seed(12345)
m_sp3 <- with(imp2,
            glm(sp_mprev_t2 ~ treat_time_t2_v1 +
                              programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini + drgs_sp_t0 + # Substance use
...
# 1 Unadjusted
m_oh1 <- glm(alco_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_oh7 <- geeglm(alco_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
log <- c("tto_primertrata_t0", "tbjo_t0", "pb_mprev_t0", "p42", 
         ...
         "redes_vivedrogas_t0", "sld_mini_antisoc_t0")
...
set.seed(12345)
m_oh3 <- with(imp2,
            glm(alco_mprev_t2 ~ treat_time_t2_v1 +
                             programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini +  drgs_sp_t0 + # Substance use
...
# 1 Unadjusted
m_mar1 <- glm(mar_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_mar7 <- geeglm(mar_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
# Dichotomous variable
log <- c("tto_primertrata_t0", "tbjo_t0", "pb_mprev_t0", "p42",
         "coc_mprev_t0", "drgs_ini", "tranq_mprev_t0",  "redes_vivedrogas_t0", "sld_mini_antisoc_t0")
...
set.seed(12345)
m_mar3 <- with(imp2,
            glm(mar_mprev_t2 ~ treat_time_t2_v1 +
                            programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
...
# 1 Unadjusted
m_coc1 <- glm(coc_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_coc7 <- geeglm(coc_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
# Dichotomous variable
log <- c("tto_primertrata_t0", "tbjo_t0", "pb_mprev_t0", "p42", 
         "tranq_mprev_t0", "drgs_ini",  "sld_mini_antisoc_t0", "redes_vivedrogas_t0")
...
set.seed(12345)
m_coc3 <- with(imp2,
            glm(coc_mprev_t2 ~ treat_time_t2_v1 +
                             programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini + drgs_sp_t0 + # Substance use
...
# 1 Unadjusted
m_pb1 <- glm(pb_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_pb7 <- geeglm(pb_mprev_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
log <- c("tto_primertrata_t0", "tbjo_t0", "redes_vivedrogas_t0", "tranq_mprev_t0", 
         "drgs_ini",  "coc_mprev_t0", "sld_mini_antisoc_t0", "p42")
...
set.seed(12345)
m_pb3 <- with(imp2,
            glm(pb_mprev_t2 ~ treat_time_t2_v1 +
                            programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini + drgs_sp_t0 + # Substance use
...
# 1 Unadjusted
m_psyc1 <- glm(psyc_comor_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_psyc7 <- geeglm(psyc_comor_t2 ~ treat_time_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
log <- c("tto_primertrata_t0", "tbjo_t0", "pb_mprev_t0", "tranq_mprev_t0", 
         "drgs_ini", "redes_vivedrogas_t0", "mar_mprev_t0", "p42", "coc_mprev_t0", "sld_mini_antisoc_t0")
...
set.seed(12345)
m_psyc3 <- with(imp2,
            glm(psyc_comor_t2 ~ treat_time_t2_v1 +
                          programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini + drgs_sp_t0 +  # Substance use
...
treat_2 <- tableby(treat_comp_t2_v1 ~ 
                     sp_mprev_t0 + sp_mprev_t2 +
                     alco_mprev_t0 + alco_mprev_t2 +
                     mar_mprev_t0 + mar_mprev_t2 +
                     coc_mprev_t0 + coc_mprev_t2 +
...
df_model <- df_model[df_model$t2==1,]
df_model <- df_model[complete.cases(df_model$treat_comp_t2_v1), ]
...
# Perform logistic regression on the complete dataset
complete_model <- multinom(treat_comp_t2_v1 ~ 
                           programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini + drgs_sp_t0 +  # Substance use
...
df_model$pr_trc_t1[df_model$treat_comp_t2_v1 == "Not Completed"] <- df_model$pr_tc1_t1 
df_model$pr_trc_t1[df_model$treat_comp_t2_v1 == "Completed" & is.na(df_model$pr_trc_t1)] <- df_model$pr_tc2_t1
df_model$pr_trc_t1[df_model$treat_comp_t2_v1 == "Currently in treatment" & is.na(df_model$pr_trc_t1)] <- df_model$pr_tc3_t1
...
# Check density across censored and uncensored T2
density_plot_t1_1 <- density(df_model$iptw_tc_t1[df_model$treat_comp_t2_v1 == "Not Completed"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$iptw_tc_t1[df_model$treat_comp_t2_v1 == "Completed"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$iptw_tc_t1[df_model$treat_comp_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Check density across censored and uncensored T2
density_plot_t1_1 <- density(df_model$pr_tc1_t1[df_model$treat_comp_t2_v1 == "Not Completed"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$pr_tc1_t1[df_model$treat_comp_t2_v1 == "Completed"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$pr_tc1_t1[df_model$treat_comp_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Check density across censored and uncensored T2
density_plot_t1_1 <- density(df_model$pr_tc2_t1[df_model$treat_comp_t2_v1 == "Not Completed"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$pr_tc2_t1[df_model$treat_comp_t2_v1 == "Completed"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$pr_tc2_t1[df_model$treat_comp_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Check density across censored and uncensored T2
density_plot_t1_1 <- density(df_model$pr_tc3_t1[df_model$treat_comp_t2_v1 == "Not Completed"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$pr_tc3_t1[df_model$treat_comp_t2_v1 == "Completed"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$pr_tc3_t1[df_model$treat_comp_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Pr actual treatment
density_plot_t1_1 <- density(df_model$pr_trc_t1[df_model$treat_comp_t2_v1 == "Not Completed"], na.rm = TRUE)
density_plot_t1_2 <- density(df_model$pr_trc_t1[df_model$treat_comp_t2_v1 == "Completed"], na.rm = TRUE)
density_plot_t1_3 <- density(df_model$pr_trc_t1[df_model$treat_comp_t2_v1 == "Currently in treatment"], na.rm = TRUE)

plot(density_plot_t1_1, main = "Pr actual treatment (compliance) at T1 Density", xlab = "Probability compliance treatment at T1", col = "blue", xlim = c(0,1), ylim = c(0,3))
lines(density_plot_t1_2, col = "red")
lines(density_plot_t1_3, col = "green")
legend("topright", legend = c("Pr=Not Completed", "Pr=Completed", "Pr=Currently in treatment"), col = c("blue", "red", "green"), lty = 1)
```
...
save(df_iptw_t1, file = "df_iptw_tc_t1_v2_sen2.Rdata")
```

## Merge df_outcome with weights
```{r}
load(file = "df_ipcw_t2_v2_sen2.Rdata")
load(file = "df_iptw_tc_t1_v2_sen2.Rdata")
...
# Check density across censored and uncensored T2
density_plot_t1_1 <- density(df_outcome$w_t[df_outcome$treat_comp_t2_v1 == "Not Completed"], na.rm = TRUE)
density_plot_t1_2 <- density(df_outcome$w_t[df_outcome$treat_comp_t2_v1 == "Completed"], na.rm = TRUE)
density_plot_t1_3 <- density(df_outcome$w_t[df_outcome$treat_comp_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Check density across censored and uncensored T2
density_plot_t1_1 <- density(df_outcome$w_t_595[df_outcome$treat_comp_t2_v1 == "Not Completed"], na.rm = TRUE)
density_plot_t1_2 <- density(df_outcome$w_t_595[df_outcome$treat_comp_t2_v1 == "Completed"], na.rm = TRUE)
density_plot_t1_3 <- density(df_outcome$w_t_595[df_outcome$treat_comp_t2_v1 == "Currently in treatment"], na.rm = TRUE)
...
# Pr actual treatment
density_plot_t1_1 <- density(df_outcome$pr_trc_t1[df_outcome$treat_comp_t2_v1 == "Not Completed" & !is.na(df_outcome$w_t_595)], na.rm = TRUE)
density_plot_t1_2 <- density(df_outcome$pr_trc_t1[df_outcome$treat_comp_t2_v1 == "Completed" & !is.na(df_outcome$w_t_595)], na.rm = TRUE)
density_plot_t1_3 <- density(df_outcome$pr_trc_t1[df_outcome$treat_comp_t2_v1 == "Currently in treatment" & !is.na(df_outcome$w_t_595)], na.rm = TRUE)

# Plot 
plot(density_plot_t1_1, main = "Pr actual treatment (compliance) at T1 Density trimmed", 
     xlab = "Pr actual treatment", col = "blue", ylim = c(0, 3), xlim = c(0,1))
lines(density_plot_t1_2, col = "red")
lines(density_plot_t1_3, col = "green")
legend("topright", legend = c("0 to 3", "4 to 7", "8 or more", "Currently in treatment"), col = c("blue", "red", "green"), lty = 1)
```
...
# 1 Unadjusted
m_sp1 <- glm(sp_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_sp7 <- geeglm(sp_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
set.seed(12345)
m_sp3 <- with(imp2,
            glm(sp_mprev_t2 ~ treat_comp_t2_v1 +
                              programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini + drgs_sp_t0 + # Substance use
...
# 1 Unadjusted
m_oh1 <- glm(alco_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_oh7 <- geeglm(alco_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
set.seed(12345)
m_oh3 <- with(imp2,
            glm(alco_mprev_t2 ~ treat_comp_t2_v1 +
                             programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini +  drgs_sp_t0 + # Substance use
...
# 1 Unadjusted
m_mar1 <- glm(mar_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_mar7 <- geeglm(mar_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
set.seed(12345)
m_mar3 <- with(imp2,
            glm(mar_mprev_t2 ~ treat_comp_t2_v1 +
                            programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini +  drgs_sp_t0 + # Substance use
...
# 1 Unadjusted
m_coc1 <- glm(coc_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_coc7 <- geeglm(coc_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
set.seed(12345)
m_coc3 <- with(imp2,
            glm(coc_mprev_t2 ~ treat_comp_t2_v1 +
                             programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini + drgs_sp_t0 + # Substance use
...
# 1 Unadjusted
m_pb1 <- glm(pb_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_pb7 <- geeglm(pb_mprev_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
set.seed(12345)
m_pb3 <- with(imp2,
            glm(pb_mprev_t2 ~ treat_comp_t2_v1 +
                            programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini + drgs_sp_t0 + # Substance use
...
# 1 Unadjusted
m_psyc1 <- glm(psyc_comor_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases)
...
# 7 Adjusted trimmed weights
m_psyc7 <- geeglm(psyc_comor_t2 ~ treat_comp_t2_v1,
                family = "binomial",
                data = df_complete_cases,
                id = folio,
                weights = w_t_595)
...
set.seed(12345)
m_psyc3 <- with(imp2,
            glm(psyc_comor_t2 ~ treat_comp_t2_v1 +
                          programa_t0 + ratio + p42 + # Center covariates
                    sd_edad_t0 + sd_sexo_t0 + educ_nivel2_t0 + tbjo_t0 + ins_hous_t0 + # Sociodemographics
                    alco_mprev_t0 + mar_mprev_t0 + coc_mprev_t0 + 
                    pb_mprev_t0 + tranq_mprev_t0 + drgs_polimes_t0 + drgs_ini + drgs_sp_t0 +  # Substance use
